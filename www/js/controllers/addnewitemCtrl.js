angular.module('app.addNewItemCtrl', ['ngCordova'])
    ...
    .controller('addNewItemCtrl', ['$scope', '$stateParams', '$state', '$cordovaSQLite', '$ionicPlatform', '$ionicPopup', '$ionicHistory', // The following is the constructor function for this page's controller. See https://docs.angularjs.org/guide/controller
        // You can include any angular dependencies as parameters for this function
        // TIP: Access Route Parameters for your page via $stateParams.parameterName
        function ($scope, $stateParams, $state, $cordovaSQLite, $ionicPlatform, $ionicPopup, $ionicHistory) {
            ...
            $ionicPlatform.ready(function () {
                ...
                $scope.insertNewItem = function () {
                    ...
                    if ($scope.item.itemname && $scope.item.soldby && $scope.item.price && $scope.item.sku) {
                        var query = "INSERT into items (itemname,category,soldby,price,sku,barcode,cashier) VALUES(?,?,?,?,?,?,?)";
                        //var query2 = "INSERT into receipt (itemname,category,soldby,price,sku,barcode,cashier,ordernumber) VALUES(?,?,?,?,?,?,?,?)";
                        $cordovaSQLite.execute(db, query, [$scope.item.itemname, $scope.item.category, $scope.item.soldby, $scope.item.price, $scope.item.sku, $scope.item.barcode, $scope.item.cashier])
                            .then(function (result) {
                                ...
                                console.log($scope.item.cashier);
